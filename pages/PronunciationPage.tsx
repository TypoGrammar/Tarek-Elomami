...
const PronunciationPage: React.FC = () => {
  usePageMetadata({
    title: 'Pronunciation Practice Tool | TypoGrammar',
    description:
      'Practice your English pronunciation. Enter any word to hear its pronunciation and see the phonetic spelling.',
  });
  ...
  return (
    <article className="max-w-4xl mx-auto bg-white p-8 md:p-12 rounded-xl shadow-lg shadow-slate-200/50 border border-slate-200 dark:bg-slate-800/50 dark:border-slate-700 dark:shadow-slate-900/50">
      <div className="text-center">
        ...
        <ArticleParagraph>
          Type an English word below and press 'Search' to hear its correct pronunciation and see
      ...
      <div className="mt-12">
        ...
        {result && (
          <div className="p-6 md:p-8 bg-slate-50 rounded-lg border border-slate-200 dark:bg-slate-800 dark:border-slate-700">
            <h2 className="font-heading text-3xl font-bold text-slate-800 mb-6 capitalize dark:text-slate-200">
              {result.word}
            </h2>
            <div className="space-y-4">
              {result.phonetics.map((p, index) => (
                <div
                  key={index}
                  className="flex items-center justify-between gap-4 p-4 bg-white rounded-md shadow-sm dark:bg-slate-700"
                >
                  <span className="font-code text-xl text-slate-700 dark:text-slate-300">
                    {p.text}
                  </span>
                  <button
